---
title: "Sự kiện 2: FCAJ x AABW Cloud Workshop - Thử Thách Kiến Trúc Dự Án Hackathon"
date: 2026-07-26
weight: 2
chapter: false
pre: " <b> 4.2. </b> "
...
#### 2.1. Team 3KA - "Hackathon Journey: Thử Thách Xây Dựng MVP Serverless Trong 24h" (`Hackathon_Journey_3KA.pptx`)
...
#### 2.2. OneTeam - "OneTeam Community Day: Thử Thách Tự Động Hóa Hạ Tầng & Quy Trình Hợp Tác" (`OneTeam_CommunityDay.pptx`)
...
#### 2.3. Nhóm Dự Án Native App - "SA Professional Native App: Thử Thách Tích Hợp Mobile Native Với Cloud Backend" (`SA_Professional_Native_App.pptx`)
...
#### 2.4. Nhóm Dự Án SignalScout - "SignalScout: Thử Thách Phân Tích Luồng Dữ Liệu Tín Hiệu Thời Gian Thực" (`SignalScout.pptx`)
...
| Nhóm / Dự án | Thử thách kiến trúc chính | Giải pháp AWS sử dụng | Kết quả đạt được | Tài liệu Slide |
| --- | --- | --- | --- | --- |
| **Team 3KA** | Triển khai nhanh MVP trong 24h & chịu tải đột biến | AWS Lambda, API Gateway, DynamoDB (Serverless) | Hạ tầng cài đặt <10 phút, chịu tải cao, chi phí tối ưu | `Hackathon_Journey_3KA.pptx` |
| **OneTeam** | Đồng bộ hóa hợp tác đa vai trò & tránh nghẽn triển khai | Terraform (IaC), AWS CodePipeline, GitHub Actions | Triển khai tự động CI/CD chuẩn tinh thần OneTeam | `OneTeam_CommunityDay.pptx` |
| **SA Native App** | Tích hợp Mobile Native với Cloud, độ trễ thấp & bảo mật | AWS Amplify, Amazon Cognito, AWS AppSync, DynamoDB | Đồng bộ thời gian thực, độ trễ <10ms, xác thực an toàn | `SA_Professional_Native_App.pptx` |
| **SignalScout** | Xử lý luồng dữ liệu tín hiệu dung lượng lớn thời gian thực | Amazon Kinesis Data Streams, AWS Lambda, Amazon S3 | Xử lý stream dữ liệu liên tục không nghẽn hệ thống | `SignalScout.pptx` |
...
- **Kho lưu trữ Slide báo cáo thử thách kiến trúc (Google Drive):** [FCAJ x AABW Drive Folder](https://drive.google.com/drive/folders/1goIcF8jRIGZczB4DBHGTsS6mp41FWmLL)
